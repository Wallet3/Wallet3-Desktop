...
const Tokens: [IToken, { [chainId: number]: string }][] = [
  [DAI, { 1: TriPool }],
  [USDC, { 1: TriPool }],
  [USDT, { 1: TriPool }],
  [sUSD, { 1: sUSDPool }],
...
export class CurveExecutor {
  ...
  fromTokens(chainId: number): IToken[] {
    return this._fromTokens.filter((v) => v[1][chainId] !== undefined).map((v) => v[0]);
  }

  forTokens(chainId: number): IToken[] {
    return this._forTokens.filter((v) => v[1][chainId] !== undefined).map((v) => v[0]);
  }
}
